<x-layouts.organizer>
    <div class="max-w-5xl mx-auto">
        ...
        <form action="{{ route('organizer.events.update', $event) }}" method="POST" enctype="multipart/form-data"
            class="space-y-8" x-data="{ isOnlineSales: {{ $event->is_online_sales_enabled ? 'true' : 'false' }} }">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div x-data
                        x-init="flatpickr($refs.endPicker, { enableTime: true, dateFormat: 'Y-m-d H:i', time_24hr: true, defaultDate: '{{ old('end_date', $event->end_date?->format('Y-m-d H:i')) }}', altInput: true, altFormat: 'd M Y, H:i', altInputClass: 'w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all bg-white cursor-pointer' })">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="4" placeholder="Enter Description"
                            class="tinymce w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">{{ old('description', $event->description) }}</textarea>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="terms" rows="4" placeholder="Enter Term & Condition"
                            class="tinymce w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">{{ old('terms', $event->terms) }}</textarea>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                <h3 class="text-lg font-bold text-gray-900 mb-6 border-b border-gray-100 pb-2">Organizer Branding</h3>

                <div class="space-y-6">
                    <div>
                        <div
                            x-data="{ organizerLogoPreview: '{{ $event->organizer_logo_path ? (Str::startsWith($event->organizer_logo_path, ['http', 'https']) ? $event->organizer_logo_path : (file_exists(public_path($event->organizer_logo_path)) ? asset($event->organizer_logo_path) : asset('storage/' . $event->organizer_logo_path))) : '' }}' }">
