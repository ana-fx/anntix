                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div x-data="{ linkType: '{{ $banner->link_url && str_contains($banner->link_url, route('events.index')) ? 'event' : 'url' }}' }">
                        ...
                        <div x-show="linkType === 'event'">
                            <select name="event_id" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all bg-white">
                                <option value="">Select an Event</option>
                                @foreach($events as $event)
                                    <option value="{{ route('events.show', $event) }}" {{ $banner->link_url == route('events.show', $event) ? 'selected' : '' }}>{{ $event->name }}</option>
                                @endforeach
                            </select>
                            <p class="text-xs text-gray-500 mt-1">Selecting an event will automatically set the banner link.</p>
                        </div>
