...
<body class="theme-event font-sans text-gray-900 antialiased bg-gray-50">
    ...
    @if (session()->has('success') || session()->has('error'))
        <div x-data="{ show: true }" x-show="show" x-transition:enter="transition ease-out duration-300"
            ...
            class="fixed top-28 right-5 z-[100] flex flex-col gap-2 max-w-sm w-full">
            @if (session('success'))
                <div class="bg-green-500 text-white px-6 py-4 rounded-xl shadow-2xl flex items-center gap-3">
                    ...
                    <button @click="show = false" class="ml-auto hover:opacity-75">
            ...
            @endif
            @if (session('error'))
                <div class="bg-rose-500 text-white px-6 py-4 rounded-xl shadow-2xl flex items-center gap-3">
                    <svg class="w-6 h-6 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
